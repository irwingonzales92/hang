...
class AuthService
{
    ...
    func registerUser(withEmail email:String, Password password:String, andUsername username:String, userCreationComplete: @escaping (_ status: Bool, _ error: Error?) -> ())
    {
        Auth.auth().createUser(withEmail: email, password: password) { (user, error) in
            ...
            let userData = ["provider": user?.providerID, "userIsInHangout": false, "userEmail": user?.email, "username": username, "userIsLeader": false, "userIsGuest": false, "userId": Auth.auth().currentUser?.uid] as [String: Any]
